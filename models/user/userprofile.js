...
module.exports = {
  ...
  changePwd: (pwd, userId) => {
    return new Promise(async (resolve, reject) => {
      ...
      db.get()
        .collection(collection.USER_COLLECTION)
        .updateOne({ _id: ObjectId(userId) }, { $set: { password: pwd } })
        .then((response) => {
          response.status = true;
          resolve(response);
        });
  ...
  getUserAddress: (userId) => {
    return new Promise((resolve, reject) => {
      db.get()
        .collection(collection.USER_COLLECTION)
        .findOne({ _id: ObjectID(userId) }, { _id: 0, address: 1 })
        .then((response) => {
  ...
  makeDefaultAddress: (addressId) => {
    return new Promise(async (resolve, reject) => {
      let address = await db
        .get()
        .collection(collection.USER_COLLECTION)
        .findOne({ "address.id": addressId }, { "address.$": 1 });
      let matchedAddress = await address.address.filter(
  ...
  walletPurchase: (total, id) => {
    return new Promise((resolve, reject) => {
      db.get()
        .collection(collection.USER_COLLECTION)
        .updateOne({ _id: ObjectID(id) }, { $set: { wallet: total } })
        .then(() => {
          resolve();
        });
    });
