  ...
  addcatOffer: (cat) => {
    return new Promise(async (resolve, reject) => {
      await db
        ...
        .collection(collection.CATEGORY_COLLECTION)
        .updateOne(
          { _id: ObjectId(cat.category) },
          { $set: { categoryOffer: parseInt(cat.offerPercentage) } }
        );
      await db
        .get()
        .collection(collection.PRODUCT_COLLECTION)
      ...
      for (let i = 0; i < products.length; i++) {
        if (products[i].categoryOffer >= products[i].productOffer) {
          ...
          await db
            ...
            .updateOne(
              { _id: ObjectId(products[i]._id) },
              { $set: { offer: products[i].categoryOffer } }
            );
          resolve();
          ...
          await db
            ...
            .updateOne(
              { _id: ObjectId(products[i]._id) },
              { $set: { offer: products[i].productOffer } }
            );
          resolve();
        }
      }
      ...
      for (let i = 0; i < products.length; i++) {
        if (products[i].categoryOffer == 0 && products[i].productOffer == 0) {
          let updateProduct = await db
            ...
            .updateOne(
              { _id: ObjectId(products[i]._id) },
              { $set: { offerPrice: products[i].price } }
            );
          await db
            .get()
            .collection(collection.PRODUCT_COLLECTION)
    ...
    return new Promise(async (resolve, reject) => {
      await db
        ...
        .updateOne(
          ...
          { $set: { productOffer: parseInt(prod.offerPercentage) } }
        );

      let product = await db
        .get()
        .collection(collection.PRODUCT_COLLECTION)
        .findOne({ _id: ObjectId(prod.product) });
      if (product.productOffer >= product.categoryOffer) {
        ...
        let updateProd = await db
          .get()
          .collection(collection.PRODUCT_COLLECTION)
          .updateOne(
            { _id: ObjectId(prod.product) },
            { $set: { offerPrice: parseInt(offerPrice) } }
          );
        await db
          .get()
          .collection(collection.PRODUCT_COLLECTION)
          .updateOne(
            { _id: ObjectId(prod.product) },
            { $set: { offer: parseInt(prod.offerPercentage) } }
          );
        resolve(updateProd);
        ...
        await db
          ...
          .updateOne(
            { _id: ObjectId(prod.product) },
            { $set: { offer: parseInt(product.categoryOffer) } }
          );
        resolve(updateProd);
      }
    });
